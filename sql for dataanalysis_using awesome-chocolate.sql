...
-- Show only the selected columns from sales table

select SaleDate, Amount, Customers from sales;

select Amount, Customers, GeoID from sales;

-- Add a calculated column with SQL, Name the new field as 'Total per box' 
...
-- Showing sales data where GEOID is g1 and order by product ID &  order of amounts desc
select * from sales
where geoid='g1'
...
-- Let's Work with People table now
